...
def typecast_and_round_output(in_tensor: torch.Tensor, decimals: int=3) -> list:
    """Moves the output to CPU, rounds the values of a tensor to a given number of decimal points
    ...
    """
    return in_tensor.cpu().to(torch.float64).round(decimals=decimals).tolist()