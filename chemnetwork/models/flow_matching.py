...
class NumericalODESolver():
    ...
    def _euler_solver(self, x_in: torch.Tensor) -> torch.Tensor:
        """Euler solver for integrating a point along a vector field defined by a neural network.
            ...
            The integration happens on the device of x_in, which therefore has to match the device
            of the model that defines the vector field.
        ...
        t_list          = torch.linspace(start=0., end=1., steps=self.integration_steps, device=x_in.device, dtype=x_in.dtype)  # Integrate on whichever device the input lives on
...
class FlowMatcher():
    ...
    def _sample_t(self, x: torch.Tensor) -> torch.Tensor:
        """Generates a tensor of random time points, uniformly drawn from the interval (0., 1.).
            ...
            The time points are drawn directly on the device and in the dtype of x, so that no
            device mismatch can occur further downstream when training on an accelerator.

            Args:
                x (torch.Tensor): A data tensor (x_0 or x_1) to get the batch size, device and dtype to correctly draw time points t.

            Returns:
                torch.Tensor: A time tensor t with shape (bs_x, 1), on the same device as x.
        """
        bs, *_      = x.shape                   # Unpacks batch size into var "bs" and remaining dims into "_"
        t_batched   = torch.rand(size=(bs, 1), device=x.device, dtype=x.dtype)
        return t_batched
