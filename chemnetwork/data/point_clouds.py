...
class PointCloudGenerator():
    ...
    def __init__(self, num_samples: int=50, target_name: str="moons", target_noise: float=0.2, device: str | torch.device="cpu"):
        """Instatiates a generator that allows drawing data from initial and target data distributions p_0 and p_1.

            Note that the samples themselves are always drawn on the CPU (scipy and sklearn are numpy based)
            and only moved to the requested device afterwards. Hence, a given seed yields the same data
            on every device.
            ...
            Args:
                ...
                device (str | torch.device, optional): Device the drawn tensors are placed on. Defaults to "cpu".
        """
    ...
    def draw_target(self) -> torch.Tensor:
        """Draws samples from the target data distribution p_1.

            Returns:
                torch.Tensor: A set of data points drawn from the chosen target distribution p_1, placed on self.device.  Shape (num_samples, 2).
        """
        dispatch = {
        ...
        return dispatch[target]().to(device=self.device)